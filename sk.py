...
class THE:
  cliffs = o(dull= [0.147, # sm1all
                    0.25, # halfway small meidan
                    0.33,  # medium
                    0.474 # large
                    ][3])
  bs=     o( conf=0.025,
...
#-------------------------------------------------------
def cohen(l1,l2):
  return abs(mid(l1) - mid(l2)) < 0.35*sd(l1+l2)
...
class Rx(Mine):
  ...
  def sk(rxs):
    ...
    def divide(lo,hi,b4,rank):
      ...
      if cut and abs(median(left.vals) - median(right.vals)) >= cohen:
      ...
      return rank
    #-- sk main
    cohen = THE.cohen*sd([x for rx in rxs for x in rx.vals])
    print(cohen)
    rxs=sorted(rxs)
